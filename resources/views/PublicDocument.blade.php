                    ...
                    <div class="card-body">
                        <form class="row g-3 align-items-center" action="{{ route('public.document') }}" method="GET">
                            ...
                            <div class="col-md-4 col-sm-6 d-flex align-items-center">
                                <input type="text" class="form-control me-2" placeholder="Search Here" name="search" value="{{ $search }}">
                            ...
                            <div class="col-md-4 col-sm-6 d-flex align-items-center">
                                <select name="category" id="category-select" class="form-control">
                                    <option value="" {{ empty($categorysearch) ? 'selected' : '' }}>Select Category</option>
                                    @foreach ($categories as $category)
                                        <option {{ $category == $categorysearch ? 'selected' : '' }} value="{{ $category }}">
                                            {{ $category }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4 col-sm-12 text-end">
                                @auth
                                    @if (auth()->user()->role === 'admin')
                                        <a href="{{ route('admin.dashboard') }}" class="btn btn-success btn-login">Dashboard</a>
                                    @else
                                        <a href="{{ route('user.dashboard') }}" class="btn btn-success btn-login">Dashboard</a>
                                    @endif
                                @endauth
                    ...
                    @foreach ($alldocuments as $item)
                        <div class="card mb-3 shadow-sm">
                            ...
                            <div class="card-body">
                                ...
                                <a href="{{ route('public.document.detail', $item->id) }}" class="text-primary">See
                                    more...</a>
